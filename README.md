...
- 원격(origin)에 새 커밋이 있으면 **`⬆ Update (N)`** 버튼이 나타납니다. 누르면 `git pull --ff-only → go build(버전 재임베드) → 바이너리 교체 → 자동 재시작`까지 처리합니다.
- `install.sh`로 설치한 메뉴바 앱도 자가 업데이트가 동작합니다 — 빌드 시 심긴 소스 체크아웃(`buildRepo`)에서 pull·빌드한 뒤 `.app` 바이너리를 교체하고 같은 PID로 재실행합니다. (단, 이 기능이 든 바이너리로 한 번 재설치한 이후부터)
- `go run`(`run.sh`/`run-web.sh`)으로 실행하면 버전은 표시되되 자가 업데이트는 비활성입니다(임시 바이너리라 교체 불가).
- 헤더/사이드바의 **버전 표시를 클릭**하면 업데이트를 즉시 다시 확인합니다(업데이트가 있으면 바로 적용).
